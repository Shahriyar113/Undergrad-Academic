...
public class Min_Priority_Queue {
    ...
    void Min_Heapify(int index)
    {
        ...
        if( l < heap_length && nodes[l].shortest_path < nodes[index].shortest_path )
        {
            smallest = l;
        }
        else
        ...
        if(r < heap_length && nodes[r].shortest_path < nodes[smallest].shortest_path)
        {
            smallest = r;
        }
    ...
    void insert_heapify(int index)
    {
        ...
        if(parent_index >= 0  && nodes[index].shortest_path < nodes[parent_index].shortest_path)
        {
            temp = nodes[index];
    ...
    void Min_heapify_dynamic(int index)
    {
        ...
        if(l < heap_length && nodes[index].shortest_path > nodes[l].shortest_path)
        {
            Min_Heapify(index); //bubble down
        }

        if(r < heap_length && nodes[index].shortest_path > nodes[r].shortest_path)
        {
            Min_Heapify(index);//bubble down
        }

        if(p >= 0 && nodes[index].shortest_path < nodes[p].shortest_path )
        {
