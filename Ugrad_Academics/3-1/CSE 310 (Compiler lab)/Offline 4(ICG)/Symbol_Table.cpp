...
private:
    ...
    string code;
    string assembly_symbol; //source code er symbol(var or array) er corresponding assembly_symbol  
...
public:
    ...
    SymbolInfo(string name,string type,string dtype,string i_type,vector<SymbolInfo*> *p,string asm_sym )
    {
        Name = name;
        Type = type;
        Next = NULL;
    ...
    void setAssemblySymbol(string asm_sym)
    {
    	assembly_symbol = asm_sym;
    }
...
//        cout << "ScopeTable with id "<< temp->get_scope_table_id() <<" removed" <<endl<<endl;
    ...
    string getCurrentScopeTableID(){
        return current_scope_table->get_scope_table_id();
    }
