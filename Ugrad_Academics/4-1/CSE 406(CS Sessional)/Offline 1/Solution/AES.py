...
def perform_AES(plain_text_blocks,original_key,Modified_key):
   
   #translating "key" to hex code
   #-------------------------------

    modified_key_hex_code = text_2_hex_code(Modified_key)

    start_time_for_key_scheduling = time.time()
    keys = Generate_all_round_keys(modified_key_hex_code)
    end_time_for_key_scheduling = time.time()

    #perform encryption and decryption for each block
    decrypted_plain_text = '' #decrypted plain text with dummy characters

    complete_plain_text_hex_code =''   #concatenating hex text of each block of plain text#
    complete_cipher_text_hex_code = '' #concatenating cipher text hex code of each block
    complete_decipher_text_hex_code = '' #concatenating decipher text hex code of each block


    for block in plain_text_blocks:
        
        #translating each "block" to  hex code 
        #------------------------------------
        block_hex_code = text_2_hex_code(block)
        complete_plain_text_hex_code = complete_plain_text_hex_code +' ' +block_hex_code

        #Generating all keys by expanding the "inputed key"

        #encrypting plain text
        start_time_for_encryption = time.time()
        cipher_text = encrypt(block_hex_code,keys)
        end_time_for_encryption = time.time()

        complete_cipher_text_hex_code = complete_cipher_text_hex_code +' '+cipher_text
        #print('Cipher_text :',cipher_text)

        #decrypting cipher text
        start_time_for_decryption = time.time()
        decrypted_block,decrypted_block_hex_code = decrypt(cipher_text,keys)
        end_time_for_decryption = time.time()

        complete_decipher_text_hex_code = complete_decipher_text_hex_code +' '+decrypted_block_hex_code
        #print('Decrypted Plain text :',plain_text)
        decrypted_plain_text = decrypted_plain_text + decrypted_block

    #print_key_of_all_round(keys)
    #print_key_of_all_round(keys.reverse())

    Final_decrepted_plain_text = decrypted_plain_text[0:len(plain_text)] #finally trancating the dummy characters which were inserted during encryption

    #print('Decrypted_plain_text: ',Final_decrepted_plain_text)


    complete_cipher_text_hex_code = complete_cipher_text_hex_code.lstrip() #trancating leading spaces
    complete_plain_text_hex_code = complete_plain_text_hex_code.lstrip() #trancating leading spaces
    complete_decipher_text_hex_code = complete_decipher_text_hex_code.lstrip() #trancating leading spaces


    #Showing output:
    print('Plain Text:')
    print('In ASCII: ',plain_text)
    print('In HEX: ',complete_plain_text_hex_code.replace(" ",""))

    print()

    print('Key:')
    print('In ASCII: ',original_key)
    print('In HEX: ',text_2_hex_code(original_key).replace(" ",""))

    print()

    print('Cipher Text:')
    print('In HEX: ',complete_cipher_text_hex_code.replace(" ","") )
    print('In ASCII: ',hex_code_2_text(complete_cipher_text_hex_code))

    print()

    print('Deciphered Text/Decrypted Text:')
    print('In HEX: ',complete_decipher_text_hex_code.replace(" ","") )
    print('In ASCII: ',Final_decrepted_plain_text)

    print()

    print('Execution time details:')
    print('Key Scheduling: ',(end_time_for_key_scheduling - start_time_for_key_scheduling),' seconds' )
    print('Encryption time(For each block): ',(end_time_for_encryption - start_time_for_encryption),' seconds' )
    print('Decryption time(For each block): ',(end_time_for_decryption - start_time_for_decryption),' seconds' )
...
plain_text = input_file.readline().rstrip() #rstrip() is used to truncate the "newline" character
original_key = input_file.readline().rstrip()
input_file.close()

 
#handling "key" if inputed_key size is larger than expected "key_length"(trancating)
Modified_key=original_key
if(len(original_key) > key_length):
    Modified_key = original_key[0:key_length]

#handling "key" if inputed_key size is smaller than expected "key_length"(adding dummy characters)
if(len(original_key) < key_length):
    #inserting dummy characters at the end
    num_of_dummy_characters = key_length - len(original_key)
    for i in range(1,num_of_dummy_characters+1):
        Modified_key = Modified_key + chr(i)
...
perform_AES(plain_text_blocks,original_key,Modified_key)
