...
public class DP_efficient {
    ...
    public static long Total_No_Of_Ways(int n,int s,int[] faces)
    {
        ...
        for(int i=2;i<=n;i++)
        {
            for(int j=i;j<=s;j++)
            {
                // DP_table[i][j] = (DP_table[i][j-1] + DP_table[i-1][j-1]) % mod; //this also same as the below line

                //To prevent the overflow "mod" is added everytime
                    DP_table[i][j] = (DP_table[i][j-1] % mod + DP_table[i-1][j-1] % mod) % mod;

                    if(j - faces[i-1] -1 >= 0)
                    {
                        DP_table[i][j] =  (DP_table[i][j]  - DP_table[i-1][j - faces[i-1] -1]);

                        if(DP_table[i][j] < 0)
                        {
                            DP_table[i][j] = DP_table[i][j] + mod;
                        }
                       // DP_table[i][j] =  DP_table[i][j] % mod;
                    }
